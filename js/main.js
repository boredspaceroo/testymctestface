...
(() => {
  ...
  async function loadLiveTLE() {
    ...
    const reasons = [];
    for (const [label, url] of [['direct', direct], ['proxy', proxied]]) {
      try {
        ...
        reasons.push(label + ': fetched OK but no matching TLE lines in response');
      } catch (err) {
        reasons.push(label + ': ' + (err.name || 'Error') + ' - ' + (err.message || 'unknown'));
      }
    }
    tleLive = false;
    tleStatusMessage = FALLBACK_NOTE;
    tleFetchDebug = reasons.join('  |  ');
    updateTleBadge();
  }
  loadLiveTLE();
